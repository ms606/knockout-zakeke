...
export interface TrayProps {
  ...
  filteredAreas: any;
  toggleFunc: (value: string) => void;
  UpdateGroupId: (data: number, type: string) => void;
  updCurrentIndex: (index: number) => void;
  selectedTray: string;
  selectStepName:  (name: string) => void;
}

const Tray: React.FC<TrayProps>
 = ({ groupNameList, filteredAreas, toggleFunc, UpdateGroupId, updCurrentIndex, selectedTray, selectStepName }) => {
  const { setItemTextOnPath, addItemText, fonts, defaultColor, items, groups, translations } = useZakeke();
  ...
  const handleMultipleClicks = (event: any) => {
    UpdateGroupId(event.target.id, "colors");
    toggleFunc("colors");
  };

  const handleTextItem = (actualAreaId: any) => {
    const itemText = {
      guid: "",
      name: "",
  ...
  const handleImageItem = (actualAreaId: any) => {
    selectStepName('')
