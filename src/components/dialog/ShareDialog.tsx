    ...
    return <Dialog
        ...
        <ShareDialogContainer>
            {(sellerSettings?.shareType === 1 || sellerSettings?.shareType === 3) && <>
                ...
                {isMobile && (navigator as any).share && <Button uppercase primary isFullWidth onClick={() => handleOnShareClick(imgUrl)}>{T._("Share image", "Composer")}</Button>}
                {(!isMobile || !navigator.share) && <SocialContainer>
                    <SocialIcon title="Facebook" onClick={() => handleFacebookClick(imgUrl)}><FbIcon /></SocialIcon>
                    <SocialIcon title="Pinterest" onClick={() => handlePinterestClick(imgUrl)}><PinIcon /></SocialIcon>
                    <SocialIcon title="Twitter" onClick={() => handleTwitterClick(imgUrl)}><TwitterIcon /></SocialIcon>
                    <SocialIcon title="Email" onClick={() => handleEmailClick(imgUrl)}><EmailIcon /></SocialIcon>
                    <SocialIcon title="Whatsapp" onClick={() => handleWhatsappClick(imgUrl)}><WhatsappIcon /></SocialIcon>
                </SocialContainer>}
            </>}

            {!isDraftEditor && !isViewerMode && sellerSettings!.shareType === 3 && <Hr />}

            {!isDraftEditor && !isViewerMode && (sellerSettings?.shareType === 2 || sellerSettings?.shareType === 3) && <>
                {shareCompositionUrl === "" && <TailSpin color="#000000" />}
                {shareCompositionUrl !== "" && <>
                    <ShareName>{T._("Share your configuration", "Composer")}</ShareName>
                    <ShareLinkContainer
                        value={shareCompositionUrl}
                    />
                    {isMobile && (navigator as any).share && <Button uppercase primary isFullWidth onClick={() => handleOnShareClick(shareCompositionUrl)}>{T._("Share link", "Composer")}</Button>}
                    {(!isMobile || !navigator.share) && <SocialContainer>
                        <SocialIcon title="Facebook" onClick={() => handleFacebookClick(shareCompositionUrl)}><FbIcon /></SocialIcon>
                        <SocialIcon title="Pinterest" onClick={() => handlePinterestClick(shareCompositionUrl)}><PinIcon /></SocialIcon>
                        <SocialIcon title="Twitter" onClick={() => handleTwitterClick(shareCompositionUrl)}><TwitterIcon /></SocialIcon>
                        <SocialIcon title="Email" onClick={() => handleEmailClick(shareCompositionUrl)}><EmailIcon /></SocialIcon>
                        <SocialIcon title="Whatsapp" onClick={() => handleWhatsappClick(shareCompositionUrl)}><WhatsappIcon /></SocialIcon>
                    </SocialContainer>}
                </>}
            </>}
